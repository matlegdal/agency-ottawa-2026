...
- **`fed.agreement_value` is cumulative per amendment** (F-3). Use `fed.vw_agreement_current` (committed) and `fed.vw_agreement_originals` (initial) — never `SUM` raw `agreement_value`. Naive sum: $921B vs correct $816B.
- **`fed.ref_number` is not unique** (F-1, 41,046 collisions). Use `(ref_number, COALESCE(recipient_business_number, recipient_legal_name))` for grouping.
...
- **2024 T3010 form revision (C-8)** — some financial fields are NULL for 2024 (removed) and others NULL for 2020–2023 (added 2024). Handle both NULL patterns when computing dependence ratio.
