...
Use the rubric in `references/rubric.md` to score each of the six dimensions on a 1–5 scale with a one-paragraph justification. The first three measure feasibility; the last three measure on-stage performance and novelty against existing public tools.
...
**Demo Resilience (1–5, higher = lower risk of embarrassing the team on stage)**
- Will the demo survive a Minister typing in a famous entity (their riding's largest charity, their own department's vendor) live?
- Is the output deterministic, or does it depend on inference (ratios with NULLs), fuzzy matching, or LLM judgment?
- Is the failure mode "empty result" or "wrong-looking result on a famous entity"? Wrong-looking is the worse failure.

**Narrative Specificity (1–5, higher = more vivid headline finding)**
- Can the punchline be written as *"\${amount} to {named entity}, who {verb phrase}"*? If yes, score ≥ 4.
- Or is it aggregate-shaped (*"top 5 vendors took 80%…"*)? Score ≤ 3.
- Slide test: does the demo's strongest moment fit on one line of a slide without a chart?

**Differentiation (1–5, higher = more novel vs. existing public tools)**
- Finish the sentence: *"You can't get this today from {existing tool} because {reason}."* Strong reason → high score.
- Does the team's contribution come from a join the source publishers do not produce themselves (cross-jurisdictional, cross-temporal, cross-dataset)? Or is it a chatbot wrapped around a familiar dashboard?
- With ~900 hackathon participants, a re-skin of `open.canada.ca` does not stand out. Be honest.
...
- `score_total` must equal `score_data + score_impl + score_fit + score_resilience + score_narrative + score_differentiation`. Range is 6–30. Do not write a derived total that disagrees with the parts.
...
2. **Six dimension blocks** with score, justification, and sub-question answers — Data, Implementation, Fit, Demo Resilience, Narrative Specificity, Differentiation
...
6. **Final score** — sum of the six dimensions (6–30) with a one-line verdict (Pursue / Pursue with caveats / Avoid)

Keep the total report under ~1,000 words (was 800 — the two extra dimensions need room). The audience is a hackathon team trying to choose; verbosity still hurts.
...
2. For each file, parse the YAML frontmatter to extract: `challenge`, `name`, `score_data`, `score_impl`, `score_fit`, `score_resilience`, `score_narrative`, `score_differentiation`, `score_total`, `verdict`. Use a tiny `awk` / `sed` snippet or read each file's first ~20 lines — do not pull the whole body into context.
...
Summary table format (use shortened headers to keep the table readable):
...
Header legend: **Res** = Demo Resilience, **Narr** = Narrative Specificity, **Diff** = Differentiation.
